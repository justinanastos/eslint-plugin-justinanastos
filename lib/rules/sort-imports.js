...
const getContentBetween = ( context, node, nodeOrTokenA, nodeOrTokenB ) => {
  const sourceCode = context.getSourceCode();
  const text = sourceCode.getText(node);

  return context.getSourceCode()
    .getText(node)
    .substr(nodeOrTokenA.end - node.start, nodeOrTokenB.start - nodeOrTokenA.end);
};
  ...
  if ( !ignoreMemberSort && node.specifiers.length > 1 ) {
    ...
    const makeFix = cSpecifier => fixer =>
      fixer.replaceTextRange(
        ...
        `${getText(cSpecifier)}${getContentBetween(context, node, pSpecifier, cSpecifier)}${getText(pSpecifier)}`
      )
