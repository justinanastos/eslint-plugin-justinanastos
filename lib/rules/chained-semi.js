...
function findFirstNodeOnThisLine(node) {
  let checkNode = node;

  do {
    if (node.loc.start.line !== checkNode.parent.loc.start.line) {
      return checkNode;
    }
...
module.exports = {
  ...
  create(context) {
    ...
    function testSemi(node) {
      ...
      function fix(fixer) {
        const column = findFirstNodeOnThisLine(node).loc.start.column;
        const whitespace = `\n${' '.repeat(column)}`;
      ...
      // console.log(findFirstNodeOnThisLine(node));

      if (findFirstNodeOnThisLine(node).loc.start.column !== semiToken.loc.start.column) {
        context.report({
          node,
